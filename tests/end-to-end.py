...
def test_crud_personas(srvparams):
    with yenot.tests.server_running(**srvparams) as server:
        ...
        perrow.memo = 'We are the pirates who don''t do anything, we just stay at home and lie around'
        hector_id = perrow.id
...
def test_crud_bits(srvparams):
    with yenot.tests.server_running(**srvparams) as server:
        ...
        content = client.get('api/personas/list')
        found = next(row for row in content.main_table().rows if row.l_name == 'Barbossa')
        ...
        client.put('api/persona/{}/bit/{}', found.id, bitrow.id, files={'bit': bittable.as_http_post_file()})
